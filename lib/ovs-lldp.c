                                   ...
                                   struct lldpd_port *port)
{
    ...
    if (port->p_dot3_enable & (1 << LLDP_TLV_DOT3_MFS)) {
        json_object_put(port_json, "mfs", json_integer_create(port->p_dot3.mfs));
    }
}
                              ...
                              struct lldpd_hardware *hw)
{
    ...
    LIST_FOR_EACH (port, p_entries, &hw->h_rports) {
        ...
        if (port->p_id_subtype == LLDP_PORTID_SUBTYPE_LLADDR) {
            ...
            if (port->p_id) {
                char *str = xmalloc(port->p_id_len + 1);
                strncpy(str, (char *) port->p_id, port->p_id_len);
                str[port->p_id_len] = '\0';  
                json_object_put(port_id_json, "value", json_string_create(str));
                free(str);
            } else {
                json_object_put(port_id_json, "value", json_string_create(none_str));
            }
        }
